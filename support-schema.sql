...
CREATE TABLE IF NOT EXISTS support_presence (
  session_id TEXT NOT NULL,
  audience TEXT NOT NULL CHECK (audience IN ('visitor', 'operator')),
...
CREATE INDEX IF NOT EXISTS idx_support_presence_session_updated
  ON support_presence (session_id, updated_at DESC);

CREATE INDEX IF NOT EXISTS idx_support_effects_created
  ON support_effects (created_at DESC);
